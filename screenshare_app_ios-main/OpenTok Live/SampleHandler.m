...
- (CIImage*) scaleFilterImage: (CIImage*)inputImage withAspectRatio:(CGFloat)aspectRatio scale:(CGFloat)scale
{
    ...
    if(IDIOM == IPAD ){
      //  NSLog(@"iPad native width rs  %f height rs %f",[[UIScreen mainScreen] bounds].size.width,[[UIScreen mainScreen] bounds].size.height);
        
        static const CGSize kTargetSize6949 = {576, 810};
        static const CGSize kTargetSize6984 = {576, 810};

        CGFloat screenWidth = CGRectGetWidth([UIScreen mainScreen].bounds);
        CGFloat screenHeight = CGRectGetHeight([UIScreen mainScreen].bounds);

        CGFloat ratio = screenWidth / screenHeight;
        CGSize targetSize = CGSizeZero; // Default to zero size

        if (fabs(ratio - 0.6949) < 0.01) {
            targetSize = kTargetSize6949;
        } else if (fabs(ratio - 0.6984) < 0.01) {
            targetSize = kTargetSize6984;
        }
        if (!CGSizeEqualToSize(targetSize, CGSizeZero)) {
           // NSLog(@"Ratio matched");

            // Calculate scale and aspect ratio based on target size
            CGFloat targetHeight = targetSize.height;
            CGFloat scale1 = targetHeight / inputImage.extent.size.height;
            CGFloat aspectRatio1 = targetSize.width / (inputImage.extent.size.width * scale1);

            [_scaleFilter setValue:inputImage forKey:kCIInputImageKey];
            [_scaleFilter setValue:@(scale1) forKey:kCIInputScaleKey];
            [_scaleFilter setValue:@(aspectRatio1) forKey:kCIInputAspectRatioKey];
        } else {
           // NSLog(@"No ratio matched, applying default scale");
            // Default scaling if no target size is matched
            [_scaleFilter setValue:inputImage forKey:kCIInputImageKey];
